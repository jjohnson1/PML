...
acc <- mean(fit$resample$Accuracy)
```

This indicates reasonable performance of the model on the training set. The model accurately predicts `r round(sum(pred == train$classe)/nrow(train)*100, 2)`% of the observations, which is right in line with expectations from the summary presented above. 

### Out of Sample Error
From the summary output of the model above, we see that the accuracy of the model is `r round(acc*100, 2)`% when trained using 10-fold cross-validation repeated 10 times. As a result, **I expect the out of sample error rate (taken to be the misclassification rate in this case) to be `r round((1-acc)*100,2)`%.** I calculate this by simply subtracting the reported accuracy from 1. 